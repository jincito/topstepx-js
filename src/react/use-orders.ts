...
/**
 * Hook that fetches orders via REST and merges real-time WebSocket updates.
 *
 ...
 *
 * @param accountId - Account to fetch orders for
 * @param startTimestamp - Start of time range (ISO string)
 * @param endTimestamp - Optional end of time range (ISO string)
 *
 * @example
 * ```tsx
 * const { data: orders, loading, error, refetch } = useOrders(100, '2024-01-01');
 * ```
 */
